...
## 4. Interpretação do Modelo (Importância das Variáveis)
Atendendo aos requisitos avançados de análise preditiva, extraímos a importância matemática das variáveis (features) utilizadas pela Floresta Aleatória. O modelo identificou que as features mais determinantes para classificar uma fraude foram o `risk_score` (calculado previamente pelo motor de regras) e o `channel_index` (canal de origem). Isso demonstra coerência analítica: o algoritmo validou de forma automatizada que transações com score elevado vindas de canais vulneráveis (como a Web) têm a probabilidade estatística quase absoluta de serem fraudes, permitindo à TechPay bloquear essas tentativas em milissegundos sem depender exclusivamente de revisão humana.
